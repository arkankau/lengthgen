...
def fig_variance_collapse(gm):
    ...
    a.legend(ncol=2, fontsize=5.5, loc="lower left")
    finish(fig, "fig_variance_collapse")


# ---- 2. pre/post variance (remedy on), 2x2 -------------------------------------
def fig_prepost(ga):
    fig, ax = plt.subplots(2, 2, figsize=(CW, 3.0), sharex=True)
...
def fig_accuracy(gm):
    ...
    fig, ax = plt.subplots(3, 2, figsize=(CW, 4.1), sharex=True)
    for i, t in enumerate(tasks):
        for j, pe in enumerate(PES):
...
def fig_causal(ga):
    ...
    fig, ax = plt.subplots(2, 1, figsize=(CW, 4.0))
