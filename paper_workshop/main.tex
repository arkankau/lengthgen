...
% Requires neurips_2026.sty from https://neurips.cc/Conferences/2026/CallForPapers
% (or start from the official NeurIPS 2026 Overleaf template and paste this body in).
\documentclass{article}
\usepackage[final]{neurips_2026}   % double-blind: keep the default (anonymous) submission mode
...
Source-max raises per-token accuracy by $0.153$, source-min lowers it by
...
orders the finite margin effect with $0.989$ Spearman correlation and $99.5\%$ sign agreement. Qwen and Pythia
show the same direction, while Gemma and SmolLM2 show saturation and selector mismatch.
...
% PAGE BUDGET: the illustrative routing schematic is Figure~\ref{fig:overview} in Appendix~\ref{app:overview}.
...
weight spectrum. We isolate the behavioral effect of assigning its available selectivity to
...
% PAGE BUDGET: protocol schematic moved to Appendix B (the text above states the protocol in full).
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_pretrained_summary.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_natural_length_ladder.png}
...
\section{Illustration of a spectrum-preserving swap}
\label{app:overview}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.97\textwidth]{figures/fig_routing_overview.png}
\caption{Question-answering use case. For the same context and question, a spectrum-preserving swap (SPS)
moves the maximum attention weight from a distractor (``Paris'') to required evidence (``Tokyo''). The model,
values, and weight spectrum remain unchanged; the paired target-answer margin tests whether assignment affects
the answer.}
\label{fig:overview}
\end{figure}

\section{Paired causal protocol}
\label{app:protocol}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.96\linewidth]{figures/fig_paired_causal_protocol.png}
