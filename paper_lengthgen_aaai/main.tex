...
Raising attention on the correct source raises accuracy in proportion, with the per-cell accuracy gain and
attention gain tracking each other at correlation $1.0$, while stabilizing the variance raises neither.
...
length, and the ratio at $50\times$ lies between $0.20$ and $0.49$ (Table~\ref{tab:main}).
...
The models are small, with four layers and width $256$.
We run the comparison where the variance collapse is strongest, and we do not establish the same behavior
at frontier scale.
We do not observe a length-generalization improvement from holding the variance constant in any cell, and
we cannot exclude that a different scale or task construction behaves differently.
