...
Figure~\ref{fig:var} shows the variance of the deepest-collapsing layer, normalized to the training
length, and the baseline ratio at $50\times$ lies between $0.27$ and $0.48$ (Table~\ref{tab:main}).
...
\caption{A co-varying statistic.
...
statistic directly.
Figure~\ref{fig:prepost} confirms the normalization works as intended.
...
\caption{The intervention holds the statistic constant.
...
\caption{Per-token accuracy against length on a log axis, out to $50\times$ the training length.
...
Figure~\ref{fig:benefit} shows the sign and the trend directly.
...
\caption{The intervention's per-token effect (intervention minus baseline) against length.
...
lower it.
Figure~\ref{fig:attnfix} compares attention on the correct source with the intervention off and on.
...
\caption{The behavior follows the operative variable.
...
exact-match $0$ throughout (Figure~\ref{fig:acc}, bottom row).
...
In our setting that interaction lowers attention on the correct source (Figure~\ref{fig:attnfix}), which is
