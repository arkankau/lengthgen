...
not test whether the per-row utility functional predicts a finite swap effect; the held-out predictor audit
reported in the supplement serves that separate purpose.
...
prediction; the supplementary held-out audit evaluates that prediction.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.96\columnwidth]{figures/fig_natural_length_ladder.png}
...
frozen Qwen calibration seeds, selected circuits are positive at every head budget: $K=2,4,8$ gives
...
\paragraph{Scope.}
The protocol needs identifiable evidence positions, disjoint calibration for utility selection, and an
off-manifold reassignment counterfactual. Higher evidence arity remains competence-limited, and natural
long-context failure has multiple causes. Within that scope, fixed-spectrum swaps make the
capacity--assignment distinction causal across controlled retrieval, pretrained models, and fixed-context natural QA.
