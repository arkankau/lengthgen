...
def fig_causal(ga):
    ...
    rsa = spearmanr(xs_a, ys).statistic; rsv = spearmanr(xs_v, ys).statistic
    kta = kendalltau(xs_a, ys).statistic; ktv = kendalltau(xs_v, ys).statistic
    fig, ax = plt.subplots(2, 1, figsize=(3.55, 4.75))
    ax[0].scatter(xs_a, ys, s=8, c=cs, alpha=0.48, edgecolors="none")
    ax[0].set_xlabel("attention on correct source"); ax[0].set_ylabel("per-token acc"); ax[0].grid(alpha=0.25)
    ax[0].set_title("(a) Source attention")
    ax[1].scatter(xs_v, ys, s=8, c=cs, alpha=0.48, edgecolors="none")
    ax[1].set_xlabel("attention-output variance ratio"); ax[1].set_ylabel("per-token acc"); ax[1].grid(alpha=0.25)
    ax[1].set_title("(b) Output variance")
    finish(fig, "fig_causal")
    print(f"  causal Spearman(attn)={rsa:.3f} variance={rsv:.3f}; "
          f"Kendall(attn)={kta:.3f} variance={ktv:.3f}")
