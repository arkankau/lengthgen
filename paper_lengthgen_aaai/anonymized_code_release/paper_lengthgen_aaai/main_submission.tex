...
\begin{abstract}
Transformers can learn retrieval rules at short context lengths and may fail as competing tokens accumulate.
...
the prediction, and they also expose saturation and selector mismatch. In controlled retrieval, a local utility
term ranks finite margin effects with $0.989$ Spearman correlation and $99.5\%$ sign agreement, but does not
calibrate their magnitude. The result is a fixed-context causal
assay for attention routing in retrieval tasks evaluated across context lengths.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Transfer and boundaries.} We test the assay across controlled tasks, pretrained models,
...
This bridges the assay to an endogenous association without establishing endogenous causality.
...
test: it asks whether the same fixed-context assay remains informative as context grows. We select the circuit
...
condition for the assay rather than a failed prediction of it.
