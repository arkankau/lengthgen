...
Our question is whether attention concentration identifies task-correct routing. At fixed concentration, does
assigning the same attention spectrum to task-required evidence change the prediction?

This question matters when performance declines with length. A transformer may learn a retrieval rule on short
sequences and then lose it as distractors accumulate. We see this pattern in copying, key-value recall, and algorithmic tasks
...
reported separately in the supplement.
...
one circuit are not treated as independent models. The supplement reports the additional Qwen seed separately.
...
changes and hierarchical intervals appear in the supplement.
...
so we use it as a local ranking signal rather than a calibrated finite-effect estimator. For the 32 controlled cell means in
...
selector results, competence-gate failures, and the anonymized code package appear in the supplement.
